...
export function SkillsTab({
  ...
  const handleAdd = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agents.failedToAddSkill);
    } finally {
      setSaving(false);
      setShowPicker(false);
    }
  ...
  const handleRemove = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agents.failedToRemoveSkill);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold">{t.agents.skillsTitle}</h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            {t.agents.skillsAssignedDesc}
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {showPicker && (
        <Dialog open onOpenChange={(v) => { if (!v) setShowPicker(false); }}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-sm">{t.agents.addSkillDialogTitle}</DialogTitle>
              <DialogDescription className="text-xs">
                {t.agents.addSkillDialogDesc}
              </DialogDescription>
            </DialogHeader>
